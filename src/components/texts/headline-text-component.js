...
export default class HeadLineTextComponent extends Component {
    static propTypes = {
    ...
    static defaultProps = {
        cId: ``,
        room: `none`,
        shade: Ht.Theme.text.headline.shade,
        alignment: Ht.Theme.text.headline.alignment,
        decoration: Ht.Theme.text.headline.decoration,
        size: Ht.Theme.text.headline.size,
        indentation: 0,
        uppercased: false,
        lowercased: false,
        color: ``
    ...
    constructor (property) {
        ...
        this.refCache = {};
        this.state = {
            adjustedStyle: Ht.Theme.text.font.headline
        };
    }
    /**
    ...
    assignComponentRef = (refName) => {
        ...
        if (Hf.DEVELOPMENT) {
            if (!Hf.isString(refName)) {
                Hf.log(`error`, `HeadlineTextComponent.assignComponentRef - Input component reference name is invalid.`);
            }
        }
        ...
        if (!Hf.isEmpty(refNames)) {
            if (Hf.DEVELOPMENT) {
                if (!refNames.every((refName) => Hf.isString(refName))) {
                    Hf.log(`error`, `HeadlineTextComponent.lookupComponentRefs - Input component reference name is invalid.`);
                } else if (!refNames.every((refName) => component.refCache.hasOwnProperty(refName))) {
                    Hf.log(`error`, `HeadlineTextComponent.lookupComponentRefs - Component reference is not found.`);
                }
            }

            componentRefs = Hf.collect(...refNames).from(component.refCache);
        } else {
            Hf.log(`error`, `HeadlineTextComponent.lookupComponentRefs - Input component reference name array is empty.`);
        }
    ...
    render () {
        ...
        if (React.Children.count(children) > 0) {
            ...
            headlineTextChildren = Hf.isEmpty(fragments) ? null : fragments;
        }
