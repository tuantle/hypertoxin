 ...
 * @description - Number field input android interface.
 *
                ...
                <TextInput
                    ...
                    onChange = {(event) => {
                        ...
                        if (Hf.isEmpty(event.nativeEvent.text)) {
                            ...
                            if (monetary) {
                                ...
                                onEditing(parseFloat(event.nativeEvent.text));
                            }
                        }
                    }}
                    onEndEditing = {(event) => {
                        if (!Hf.isEmpty(event.nativeEvent.text)) {
                            ...
                            if (monetary) {
                                fieldTextInput.setNativeProps({
                                    text: `${parseFloat(event.nativeEvent.text).toFixed(2)}`
                                });
                            } else {
                                fieldTextInput.setNativeProps({
                                    text: `${parseFloat(event.nativeEvent.text)}`
                                });
                            }
                        }
                    }}
                    onSubmitEditing = {(event) => {
                        if (!Hf.isEmpty(event.nativeEvent.text)) {
                            ...
                            if (monetary) {
                                onDoneEdit(parseFloat(event.nativeEvent.text).toFixed(2));
                            } else {
                                onDoneEdit(parseFloat(event.nativeEvent.text));
                            }
                        }
                    }}
                />
    ...
    render: function render () {
        ...
        if (!Hf.isEmpty(iconPreset) && icon === null) {
            if (Ht.Theme.icon.hasOwnProperty(Hf.dashToCamelcase(iconPreset))) {
                ...
                Hf.log(`warn1`, `NumberFieldInterface - Icon preset:${iconPreset} is not found.`);
            }
        }
