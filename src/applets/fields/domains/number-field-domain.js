 ...
 *------------------------------------------------------------------------
 *
 * @module NumberFieldDomain
 * @description - Number field applet domain.
 *
...
import NumberFieldStore from '../stores/field-store';

import NumberFieldInterface from '../interfaces/number-field-interface';

import EVENT from '../events/field-event';

const NumberFieldDomain = Hf.Domain.augment({
    $init: function $init () {
        const domain = this;
        domain.register({
            store: NumberFieldStore({
                name: `number-field-store`
            }),
            intf: NumberFieldInterface({
                name: domain.name
            })
        });
    },
...
export default NumberFieldDomain;
