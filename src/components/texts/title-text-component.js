...
export default class TitleTextComponent extends Component {
    ...
    static defaultProps = {
        cId: ``,
        room: `none`,
        shade: Ht.Theme.text.title.shade,
        alignment: Ht.Theme.text.title.alignment,
        decoration: Ht.Theme.text.title.decoration,
        size: Ht.Theme.text.title.size,
        indentation: 0,
        uppercased: false,
        lowercased: false,
        color: ``
    ...
    constructor (property) {
        ...
        this.refCache = {};
        this.state = {
            adjustedStyle: Ht.Theme.text.font.title
        };
    }
    /**
        ...
        if (!Hf.isEmpty(refNames)) {
            if (Hf.DEVELOPMENT) {
                if (!refNames.every((refName) => Hf.isString(refName))) {
                    Hf.log(`error`, `TitleTextComponent.lookupComponentRefs - Input component reference name is invalid.`);
                } else if (!refNames.every((refName) => component.refCache.hasOwnProperty(refName))) {
                    Hf.log(`error`, `TitleTextComponent.lookupComponentRefs - Component reference is not found.`);
                }
            }

            componentRefs = Hf.collect(...refNames).from(component.refCache);
        } else {
            Hf.log(`error`, `TitleTextComponent.lookupComponentRefs - Input component reference name array is empty.`);
        }
    ...
    render () {
        ...
        if (React.Children.count(children) > 0) {
            ...
            titleTextChildren = Hf.isEmpty(fragments) ? null : fragments;
        }
